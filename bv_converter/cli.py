...
def process_bili_cache(cache_root, export_dir):
    ...
    for subdir in cache_path.rglob("*"):
        ...
        if len(m4s_files) >= 2 and info_file.exists():
            ...
            try:
                ...
                title = info.get("title") or info.get("data", {}).get(
                    "title", "unknown_title"
                )
    ...
    # 输出处理结果
    _logger.info(
        f"处理完成！成功导出 {len(success_names)} 个文件, 失败 {len(fail_names)} 个文件。"
    )
    if success_names:
...
def main():
    ...
    parser = argparse.ArgumentParser(description="将B站缓存中的.m4s文件转换为.mp4格式")
